...
public class RoundProgressBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (textIsDisplayable && style == STROKE) {
            //画出进度百分比
            canvas.drawText(percent + "%", centre - textWidth / 2, centre + textSize / 2 + textHeight / 2, paint);
        }
        // 正确率
        paint.setColor(textColor);
        paint.setTextSize(30);
        paint.setTypeface(Typeface.DEFAULT);
        textWidth = paint.measureText("正确率");
        canvas.drawText("正确率", centre - textWidth / 2, centre + textSize / 2 - textHeight / 2, paint);

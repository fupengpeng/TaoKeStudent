 ...
 * 解除绑定账号界面主导器
 */
public class RelieveAccountActivityPresenter extends BaseActivityPresenter {
    /* 解除绑定账号界面 */
    private RelieveAccountActivity activity;
    /* 用户信息业务 */
    private UserModel userModel;

    public RelieveAccountActivityPresenter(RelieveAccountActivity activity) {
        super(activity);
     ...
     * @param password    密码
     */
    public void relieveAccount(String phoneNumber, String password) {
        // 显示等待对话框
        showWaitDialog(Consts.WaitDialogMessage.BINDING);
        // 解除绑定账号
        userModel.relieveAccount(MyApplication.getInstance().getUser().getUid(), phoneNumber, password,
